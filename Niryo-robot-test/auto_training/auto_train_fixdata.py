...
if __name__ == "__main__":
    ...
    with open("parameters.yml",'r') as file:
        ...
        pparameters = project_parameters["auto_training_fixdata"]
        NN_parameters = project_parameters["W2qLastTokenNN"]
    project_name = pparameters["project_name"]
    fixed_win = pparameters["fixed_win"]
...
# Begin Training
    ...
    for i, win in enumerate(wins):
        ...
        X_ver,X_te,y_ver,y_te = train_test_split(X_r, y_r, test_size=0.5,shuffle=True, random_state=42)
        ...
        for epoch in pbar:
            ...
            loss.backward()
            optimizer.step()
            train_losses.append([loss.item(),reg_loss.item(),cls_loss.item()])
            # —— 2. 验证 —— #
            model.eval()
            with torch.no_grad():
...
## Test and Draw
        if i == 0:
            ...
            losses_path = os.path.join(save_folder,"training_losses")
        ...
        train_losses = np.array(train_losses).squeeze().transpose(1,0)
        np.savez(os.path.join(losses_path,f"training_loss{i}.npz"),train_losses=train_losses[0],reg_train_losses=train_losses[1],cls_train_losses=train_losses[2])
        ...
        ax.set_xlim([0,len(train_losses[0])])
        ...
        ax.plot(range(2,win+1),best_train_losses,"--o")
        ax.grid(True)
        ax.set_title("Best Training Loss")
        ax = fig.add_subplot(312)
        ax.plot(range(2,win+1),best_reg_losses["val"],"--o")
        ax.grid(True)
        ax.set_title("Best Regression Loss")
        ax = fig.add_subplot(313)
        ax.plot(range(2,win+1),best_cls_losses["val"],"--o")
        ax.grid(True)
